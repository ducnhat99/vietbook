<?php
	...
	if(!empty($_POST))
	{
		...
		if(empty($_POST['tnd']) || empty($_POST['tk']) || empty($_POST['mk']) || empty($_POST['nlmk']) || empty($_POST['sdt']) || empty($_POST['dc']) || empty($_POST['em'])||empty($_POST['ns']) ||empty($_POST['gt']))
		{
		...
		else
		{
                        ...
                        if(!empty($row))
			{
                            if($_POST['tk']==$row['usename'])
				{
                            ...
                            $query="insert into khachhang(tenkh,usename,password,diachikh,sdtkh,emailkh,ngaysinh,gioitinh,ngaydangky)
                                values('$tnd','$tk','$mk','$dc','$sdt','$em','$ns','$gt','$ngaydk')";
                                mysqli_set_charset($conn, 'UTF8');
